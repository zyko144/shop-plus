...
export function AudioPlayer() {
  ...
  return (
    <div className="fixed bottom-4 left-4 z-[100] bg-black/60 backdrop-blur-xl rounded-full px-4 py-2.5 flex items-center gap-3 border border-white/10 shadow-[0_0_20px_rgba(255,255,255,0.2)]">
      <button 
        onClick={togglePlay} 
        className="w-8 h-8 rounded-full bg-gray-600 hover:bg-gray-500 text-white flex items-center justify-center transition-colors shrink-0 shadow-lg shadow-gray-600/30"
      >
        {isPlaying ? <Pause size={16} fill="currentColor" /> : <Play size={16} fill="currentColor" className="ml-1" />}
      </button>
      
      <div className="flex flex-col">
        <div className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-0.5">En cours</div>
      ...
      <div className="flex items-center gap-2">
        ...
        <input 
          ...
          className="w-16 md:w-20 h-1.5 rounded-full bg-white/10 appearance-none cursor-pointer accent-gray-600"
        />
      </div>
