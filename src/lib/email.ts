...
// You will need to set RESEND_API_KEY in your .env
const resendApiKey = process.env.RESEND_API_KEY;

export const sendOrderEmail = createServerFn({ method: "POST" })
  .validator((data: { email: string; items: any[]; total: number }) => data)
  .handler(async ({ data }) => {
    if (!resendApiKey) {
    ...
    const itemsHtml = data.items.map(item => 
      `<li><strong>${item.quantity}x ${item.name}</strong> ${item.subtitle ? `(${item.subtitle})` : ''} - ${(item.price * item.quantity).toFixed(2)}€</li>`
    ).join('');
